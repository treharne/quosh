...
/// Settling interval before a fully written input sequence becomes an echo
/// checkpoint. A heuristic, not proof the application processed the input.
...
/// Sequenced pending PTY writes. Message boundaries let the owner know when an
/// input sequence has been fully written, which is what the echo checkpoint is
/// measured from; generated replies share an unsequenced chunk.
#[derive(Default)]
...
impl PendingWrites {
    ...
    fn push_input(&mut self, seq: u64, data: Vec<u8>) -> bool {
        if self.unread.saturating_add(data.len()) > PENDING_HARD_MAX {
            return false;
        }
        self.unread += data.len();
        self.chunks.push_back(WriteChunk {
            seq: Some(seq),
            data,
            off: 0,
        });
    ...
    /// Append terminal-generated bytes. False means the hard cap was hit; the
    /// owner must terminate rather than grow without bound.
    fn push_generated(&mut self, bytes: &[u8]) -> bool {
        // The common case is no generated reply at all. Do not enqueue an
        // empty chunk: `unread` would stay 0 but a later input chunk behind it
        // would make `write_buf` empty while `want_write` was true.
        if bytes.is_empty() {
            return true;
        }
        if self.unread.saturating_add(bytes.len()) > PENDING_HARD_MAX {
            return false;
        }
        ...
        if let Some(back) = self.chunks.back_mut()
            && back.seq.is_none()
        {
            back.data.extend_from_slice(bytes);
            return true;
        }
        self.chunks.push_back(WriteChunk {
            seq: None,
    ...
    /// Consume `n` written bytes, recording fully written input sequences.
    fn advance(&mut self, n: usize, history: &mut VecDeque<(u64, Instant)>, now: Instant) {
        let mut left = n;
        while left > 0 {
            let Some(front) = self.chunks.front_mut() else {
                break;
            };
            let avail = front.data.len() - front.off;
...
/// Advance the echo checkpoint to the newest input sequence fully written at
/// least [`ECHO_TIMEOUT`] ago. Returns true if it moved.
fn advance_echo_ack(
    echo_ack: &mut u64,
    history: &mut VecDeque<(u64, Instant)>,
    now: Instant,
) -> bool {
    let mut newest = *echo_ack;
    while let Some(&(seq, at)) = history.front() {
        if at + ECHO_TIMEOUT <= now {
            newest = seq;
            history.pop_front();
        } else {
            break;
...
impl Session {
    ...
    pub(crate) fn publish_screen_blob(&self, blob: Vec<u8>) {
        ...
        let _ = self
            .latest
            .send_replace(Some(Arc::new(Latest { version, blob })));
    }
...
impl Owner {
    async fn run(self, registry: Registry) {
        ...
        let mut history: VecDeque<(u64, Instant)> = VecDeque::new();
        let mut echo_ack: u64 = 0;
        loop {
            if sess.is_shutdown() {
                break;
            }
            ...
            let echo_at = history
                .front()
                .map(|(_, at)| tokio::time::Instant::from_std(*at + ECHO_TIMEOUT));
            tokio::select! {
                ...
                // Generated DSR/DA replies still hit the pending hard cap.
                r = afd.readable() => {
                    let n = {
                        let mut g = match r {
                            Ok(g) => g,
                    ...
                    if !pending.push_generated(&driver.take_pty_write()) {
                        tracing::error!("pty-generated writes exceeded pending budget");
                        break;
                    }
                    publish(&sess, &mut driver, &mut version, echo_ack);
                }
                r = afd.writable(), if want_write => {
                    ...
                    match g.try_io(|inner| inner.get_ref().write(pending.write_buf())) {
                        Ok(Ok(0)) | Ok(Err(_)) => break,
                        Ok(Ok(n)) => pending.advance(n, &mut history, Instant::now()),
                        Err(_would_block) => {}
                    }
                }
                msg = cmd_rx.recv(), if accept_more => {
                    match msg {
                        ...
                        Some(OwnerMsg::Input { seq, data }) => {
                            ...
                            if seq == next {
                                if !pending.push_input(seq, data) {
                ...
                // A checkpoint advancing on a quiet link still has to reach the
                // client, so re-publish the same frame with the new echo_ack.
                _ = async {
                    match echo_at {
                        Some(t) => tokio::time::sleep_until(t).await,
                        None => std::future::pending::<()>().await,
                    ...
                    if advance_echo_ack(&mut echo_ack, &mut history, Instant::now()) {
                        publish(&sess, &mut driver, &mut version, echo_ack);
                    }
                }
            }
...
fn publish(sess: &Session, driver: &mut Emulator, version: &mut u64, echo_ack: u64) {
    *version += 1;
...
mod tests {
    ...
    fn try_input_rejects_empty_and_oversized() {
        ...
        assert!(
            stub.sess
                .try_input(1, vec![0; MAX_INPUT_BYTES + 1])
                .is_err()
        );
    }
    ...
    fn generated_writes_fail_when_exceeding_hard_cap() {
        let mut pending = PendingWrites::default();
        assert!(pending.push_generated(&vec![0u8; PENDING_HARD_MAX]));
        assert!(!pending.push_generated(&[1]));
        let mut pending = PendingWrites::default();
        assert!(pending.push_generated(&vec![0u8; PENDING_HARD_MAX - 8]));
        assert!(pending.push_generated(&[1; 8]));
        assert!(!pending.push_generated(&[1]));
    }
    ...
    fn dsr_burst_is_subject_to_pending_hard_cap() {
        ...
        let mut pending = PendingWrites::default();
        assert!(pending.push_generated(&vec![0u8; PENDING_HARD_MAX - 1]));
        assert!(
            !pending.push_generated(&replies),
            "generated DSR replies must not grow pending past the hard cap"
        );
    }

    #[test]
    fn echo_ack_advances_after_completed_writes_settle() {
        let t0 = Instant::now();
        let mut pending = PendingWrites::default();
        let mut history = VecDeque::new();
        let mut echo_ack = 0;

        assert!(pending.push_input(1, b"ab".to_vec()));
        assert!(pending.push_input(2, b"c".to_vec()));
        assert_eq!(pending.unread(), 3);

        // Writing only part of message 1 completes nothing.
        pending.advance(1, &mut history, t0);
        assert!(history.is_empty());
        assert!(!advance_echo_ack(
            &mut echo_ack,
            &mut history,
            t0 + ECHO_TIMEOUT
        ));

        // Finishing message 1 records it one settling interval later.
        pending.advance(1, &mut history, t0);
        assert_eq!(history.len(), 1);
        assert!(!advance_echo_ack(
            &mut echo_ack,
            &mut history,
            t0 + ECHO_TIMEOUT - Duration::from_millis(1)
        ));
        assert!(advance_echo_ack(
            &mut echo_ack,
            &mut history,
        ...
        assert_eq!(echo_ack, 1);

        // Message 2 is still pending until its own settle deadline.
        pending.advance(1, &mut history, t0 + Duration::from_millis(10));
        assert!(!advance_echo_ack(
            &mut echo_ack,
            &mut history,
            t0 + ECHO_TIMEOUT
        ...
        assert!(advance_echo_ack(
            &mut echo_ack,
            &mut history,
            t0 + Duration::from_millis(10) + ECHO_TIMEOUT
        ));
        assert_eq!(echo_ack, 2);
    }

    #[test]
    fn empty_generated_push_does_not_create_an_empty_chunk() {
        // `take_pty_write()` is empty on almost every read. Enqueueing an empty
        // chunk made the owner see unread > 0 with an empty front buffer and
        // treat the PTY write's 0 as EOF.
        let mut pending = PendingWrites::default();
        assert!(pending.push_generated(&[]));
        assert_eq!(pending.unread(), 0);
        assert!(pending.push_input(1, b"x".to_vec()));
        assert_eq!(pending.write_buf(), b"x");
    }

    #[test]
    fn generated_bytes_do_not_extend_a_sequenced_chunk() {
        let t0 = Instant::now();
        let mut pending = PendingWrites::default();
        let mut history = VecDeque::new();
        assert!(pending.push_input(1, b"a".to_vec()));
        assert!(pending.push_generated(b"R"));
        assert_eq!(pending.write_buf(), b"a");
        pending.advance(1, &mut history, t0);
        assert_eq!(history.len(), 1, "message 1 completes before the reply");
        assert_eq!(pending.write_buf(), b"R");
        pending.advance(1, &mut history, t0);
        assert_eq!(history.len(), 1, "generated bytes are never acknowledged");
    }
    ...
    #[tokio::test]
    async fn echo_ack_reaches_the_published_screen() {
        let uid = Uid::current().as_raw();
        let id = [0x5a; 16];
        let (sess, owner) = spawn_pair(id, [0x5b; 32], uid, 80, 24).expect("spawn");
        let registry: Registry = Arc::new(Mutex::new(HashMap::new()));
        registry.lock().await.insert(id, sess.clone());
        sess.start(registry.clone(), owner);

        let mut latest = sess.subscribe_latest();
        assert!(matches!(
            sess.try_input(1, b"x".to_vec()),
            Ok(QueueResult::Queued)
        ));
        let screen = tokio::time::timeout(Duration::from_secs(3), async {
            loop {
                let snap = latest.borrow_and_update().clone();
                if let Some(l) = snap
                    && let Ok(screen) = Screen::decode_compressed(&l.blob)
                    && screen.echo_ack >= 1
                {
                    return screen;
                }
                if latest.changed().await.is_err() {
                    panic!("latest sender closed");
                }
            }
        })
        .await
        .expect("no screen with echo_ack >= 1");
        assert_eq!(screen.echo_ack, 1);
